 var myArray = [];//global variables
 var getArray; 
 //this function is execute when page is loading 
...
function loadTable() {
    ...
    myArray.forEach(e => {
        ...
        '<td><button class="dltbtn" onclick="delData('+e.userId+')">Del</button></td></tr>');
        });
}
...
function userFormDetails() {
    var record = myArray.find(obj=>obj.userId == userId.value);  
    ...
    if(record==null || record==undefined){
        ...
        myArray.forEach(o=>{
            if(o.userId == userId.value){
                o.firstName = firstName.value;
                o.lastName= lastName.value;
                o.email= email.value;
                o.mobile= mobile.value;
                o.gender= gender();
                o.address= city.value+','+state.value+','+pincode.value;
                o.country= country.value;
                o.college = college.value;
                o.degree= degree.value;
                o.percentage= percentage.value;
                o.jobArea= jobArea.value;
                o.position= position.value;
                o.skills= skills.value;
            }
        });
    }
    ...
    var getArray = JSON.parse(getString);
    myArray = getArray;
    loadTable();
...
function editData(e) {
    ...
    if(editObj.gender == "Male"){
        document.getElementById("male").checked = true;
    }else if(editObj.gender =="female"){
        document.getElementById("male").checked = true;
    }else if(editObj.gender =="others"){
        document.getElementById("male").checked = true;
    }
    var A = editObj.address;
    var addressSplit = A.split(',');
    document.getElementById("city").value = addressSplit[0];
    document.getElementById("state").value = addressSplit[1];
    document.getElementById("pincode").value = addressSplit[2];
    ...
    document.getElementById("area-select").value = editObj.jobArea;
...
function delData(userIdToDelete) 
{
    var get = window.localStorage.getItem("userDetails")
    var usrdetails = JSON.parse(get)
    var a = usrdetails.findIndex(a=>a.userId==userIdToDelete)
    usrdetails.splice(a,1);
    console.log(usrdetails)

    var c = JSON.stringify(usrdetails)
    window.localStorage.setItem("userDetails",c)    
}
